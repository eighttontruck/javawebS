...
public class MemberController {
	...
	public String memberPwdUpdateGet(HttpSession session,
			@RequestParam(name="pwdFlag", defaultValue = "", required=false) String pwdFlag) {
	...
	public String memberPwdUpdatePost(
		...
		String prePwd = passwordEncoder.encode(pwd);
		newPwd = passwordEncoder.encode(newPwd);
		
		if(!currentPwd.equals(prePwd)) return "redirect:/message/memberPwdCheckNo";
